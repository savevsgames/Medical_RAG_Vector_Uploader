/*
  ...
  This migration recreates the entire database schema with all security fixes applied.
  
  ## Key Features
  1. **Shared Medical Knowledge Base** - All doctors can access all uploaded documents
  2. **Security Fixes** - All functions have fixed search paths (resolves warnings)
  3. **Performance Optimized** - Proper indexes for vector search and queries
  4. **Agent Management** - Complete TxAgent session lifecycle management
  5. **Embedding Jobs** - Document processing job tracking
  
  ## Security Changes
  - Fixed "Function Search Path Mutable" warnings by adding `SET search_path TO public, pg_catalog;`
  - Changed document access from user-isolated to shared knowledge base
  - Maintained proper RLS for uploads (users can only upload as themselves)
  
  ## Breaking Changes
  - Documents are now readable by ALL authenticated users (shared knowledge base)
  - This transforms the app from isolated user docs to collaborative medical knowledge
*/

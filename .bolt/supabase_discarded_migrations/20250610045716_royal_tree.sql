/*
  # Complete Database Cleanup and Agent Flow Fix

  This migration completely cleans up all existing functions and policies,
  then recreates everything with proper SECURITY DEFINER and correct method names.

  ## Issues Fixed:
  1. RLS policies preventing agent session creation
  2. Function name mismatches (startAgent vs createAgentSession)
  3. Missing SECURITY DEFINER causing auth.uid() to return NULL
  4. Incorrect function volatility classifications
  5. Search path vulnerabilities
*/
...
CREATE POLICY "Allow agent inserts"
    ...
    WITH CHECK ((role() = 'service_role'::text) OR (auth.uid() = user_id));
